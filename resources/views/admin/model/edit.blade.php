                                ...
                                <div class="form-group">
                                    ...
                                    <select name="engine_id" class="form-control" required>
                                        @foreach($engines as $engine)
                                            <option value="{{$engine->id}}" @if($engine->id == $models->engine_id) selected @endif>{{$engine->engine_type}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label>Модель</label>
                                    <input value="{{$models->name}}" type="text" name="name" class="form-control" id="exampleInputCategory"
                                           placeholder="Введите название модели" required>
                                </div>
                                <div class="form-group">
                                    <label for="photo" class="form-label">Фото:</label>
                                    <input value="{{'/storage/img/'.$models->photo}}" type="file" class="form-control" id="photo"
                                           name="photo">
                                </div>
                            </div>
                            <!-- /.card-body -->
